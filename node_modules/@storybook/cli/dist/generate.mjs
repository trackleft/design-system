...
`,"").trim();await fse.writeFile(previewPath,preview,{encoding:"utf8"})}import{writeFile,pathExists}from"fs-extra";import{logger as logger3}from"@storybook/node-logger";import path from"path";import prompts from"prompts";import chalk from"chalk";var generateStorybookBabelConfigInCWD=async()=>{let target=process.cwd();return generateStorybookBabelConfig({target})},generateStorybookBabelConfig=async({target})=>{logger3.info(`Generating the storybook default babel config at ${target}`);let fileName=".babelrc.json",location=path.join(target,fileName);if(await pathExists(location)){let{overwrite}=await prompts({type:"confirm",initial:!1,name:"overwrite",message:`${fileName} already exists. Would you like overwrite it?`});if(overwrite===!1){logger3.warn("Cancelled, babel config file was NOT written to file-system.");return}}logger3.info(`The config will contain ${chalk.yellow("@babel/preset-env")} and you will be prompted for additional presets, if you wish to add them depending on your project needs.`);let{typescript,jsx}=await prompts([{type:"confirm",initial:!1,name:"typescript",message:"Do you want to add the TypeScript preset?"},{type:"confirm",initial:!1,name:"jsx",message:"Do you want to add the React preset?"}]),added=["@babel/preset-env"],presets=[["@babel/preset-env",{targets:{chrome:100}}]];typescript&&(added.push("@babel/preset-typescript"),presets.push("@babel/preset-typescript")),jsx&&(added.push("@babel/preset-react"),presets.push("@babel/preset-react"));let contents=JSON.stringify({sourceType:"unambiguous",presets,plugins:[]},null,2);logger3.info(`Writing file to ${location}`),await writeFile(location,contents);let{runInstall}=await prompts({type:"confirm",initial:!0,name:"runInstall",message:`Shall we install the required dependencies now? (${added.join(", ")})`});runInstall&&(logger3.info("Installing dependencies..."),JsPackageManagerFactory.getPackageManager().addDependencies({installAsDevDependencies:!0},added))};var defaultOptions={extraPackages:[],extraAddons:[],staticDir:void 0,addScripts:!0,addMainFile:!0,addComponents:!0,addBabel:!1,addESLint:!1,extraMain:void 0,framework:void 0,extensions:void 0,componentsDestinationPath:void 0,storybookConfigFolder:".storybook"},getBuilderDetails=builder=>{let map=versions_default;if(map[builder])return builder;let builderPackage=`@storybook/${builder}`;return map[builderPackage]?builderPackage:builder},getExternalFramework=framework=>externalFrameworks.find(exFramework=>framework!==void 0&&(exFramework.name===framework||exFramework.packageName===framework||exFramework?.frameworks?.some?.(item=>item===framework))),getFrameworkPackage=(framework,renderer,builder)=>{let externalFramework=getExternalFramework(framework);return externalFramework===void 0?framework?`@storybook/${framework}`:`@storybook/${renderer}-${builder}`:externalFramework.frameworks!==void 0?externalFramework.frameworks.find(item=>item.match(new RegExp(`-${builder}`))):externalFramework.packageName},getRendererPackage=(framework,renderer)=>{let externalFramework=getExternalFramework(framework);return externalFramework!==void 0?externalFramework.renderer||externalFramework.packageName:`@storybook/${renderer}`},wrapForPnp=packageName=>`%%path.dirname(require.resolve(path.join('${packageName}', 'package.json')))%%`,getFrameworkDetails=(renderer,builder,pnp,framework)=>{let frameworkPackage=getFrameworkPackage(framework,renderer,builder),frameworkPackagePath=pnp?wrapForPnp(frameworkPackage):frameworkPackage,rendererPackage=getRendererPackage(framework,renderer),rendererPackagePath=pnp?wrapForPnp(rendererPackage):rendererPackage,builderPackage=getBuilderDetails(builder),builderPackagePath=pnp?wrapForPnp(builderPackage):builderPackage,isKnownFramework=!!getExternalFramework(frameworkPackage)||!!versions_default[frameworkPackage],isKnownRenderer=!!versions_default[rendererPackage];if(isKnownFramework)return{packages:[rendererPackage,frameworkPackage],framework:frameworkPackagePath,rendererId:renderer,type:"framework"};if(isKnownRenderer)return{packages:[rendererPackage,builderPackage],builder:builderPackagePath,renderer:rendererPackagePath,rendererId:renderer,type:"renderer"};throw new Error(`Could not find the framework (${frameworkPackage}) or renderer (${rendererPackage}) package`)},stripVersions=addons=>addons.map(addon=>getPackageDetails(addon)[0]),hasInteractiveStories=rendererId=>["react","angular","preact","svelte","vue","vue3","html","solid"].includes(rendererId),hasFrameworkTemplates=framework=>["angular","nextjs"].includes(framework);async function baseGenerator(packageManager,npmOptions,{language,builder="webpack5",pnp,frameworkPreviewParts},renderer,options=defaultOptions,framework){let{extraAddons:extraAddonPackages,extraPackages,staticDir,addScripts,addMainFile,addComponents,addBabel,addESLint,extraMain,extensions,storybookConfigFolder,componentsDestinationPath}={...defaultOptions,...options},{packages:frameworkPackages,type,rendererId,framework:frameworkInclude,builder:builderInclude}=getFrameworkDetails(renderer,builder,pnp,framework),addons=["@storybook/addon-links","@storybook/addon-essentials",...stripVersions(extraAddonPackages)],addonPackages=["@storybook/addon-links","@storybook/addon-essentials","@storybook/blocks",...extraAddonPackages];hasInteractiveStories(rendererId)&&(addons.push("@storybook/addon-interactions"),addonPackages.push("@storybook/addon-interactions","@storybook/testing-library@^0.0.14-next.1"));let files=await fse2.readdir(process.cwd()),packageJson=packageManager.retrievePackageJson(),installedDependencies=new Set(Object.keys({...packageJson.dependencies,...packageJson.devDependencies}));if(installedDependencies.has("react")||addonPackages.push("react"),installedDependencies.has("react-dom")||addonPackages.push("react-dom"),type==="renderer")throw new Error(dedent2`
      ...
      `);let packages=["storybook",getExternalFramework(rendererId)?void 0:`@storybook/${rendererId}`,...frameworkPackages,...addonPackages,...extraPackages].filter(Boolean).filter(packageToInstall=>!installedDependencies.has(getPackageDetails(packageToInstall)[0])),versionedPackages=await packageManager.getVersionedPackages(packages);if(await fse2.ensureDir(`./${storybookConfigFolder}`),addMainFile&&await configureMain({framework:{name:frameworkInclude,options:options.framework||{}},storybookConfigFolder,docs:{autodocs:"tag"},addons:pnp?addons.map(wrapForPnp):addons,extensions,language,...staticDir?{staticDirs:[path2.join("..",staticDir)]}:null,...extraMain,...type!=="framework"?{core:{builder:builderInclude}}:{}}),await configurePreview({frameworkPreviewParts,storybookConfigFolder,language}),frameworkPackages.find(pkg2=>pkg2.match(/^(@storybook\/|storybook).*-vite$/)||pkg2==="@storybook/sveltekit"||pkg2==="")){let previewHead=dedent2`
      <script>
...
`.trimStart(),promptForCompoDocs=async()=>{let{useCompoDoc}=await prompts2({type:"confirm",name:"useCompoDoc",message:"Do you want to use Compodoc for documentation?"});return useCompoDoc},AngularJSON=class{constructor(){if(!fs2.existsSync(ANGULAR_JSON_PATH))throw commandLog("An angular.json file was not found in the current directory. Storybook needs it to work properly."),new Error("No angular.json file found");let jsonContent=fs2.readFileSync(ANGULAR_JSON_PATH,"utf8");this.json=JSON.parse(jsonContent)}get projects(){return this.json.projects}get projectsWithoutStorybook(){return Object.keys(this.projects).filter(projectName=>{let{architect}=this.projects[projectName];return!architect.storybook})}getProjectSettingsByName(projectName){return this.projects[projectName]}async getProjectName(){if(this.projectsWithoutStorybook.length>1){let{projectName}=await prompts2({type:"select",name:"projectName",message:"For which project do you want to generate Storybook configuration?",choices:this.projectsWithoutStorybook.map(name=>({title:name,value:name}))});return projectName}return this.projectsWithoutStorybook[0]}addStorybookEntries({angularProjectName,storybookFolder,useCompodoc,root}){let{architect}=this.projects[angularProjectName],baseOptions={configDir:storybookFolder,browserTarget:`${angularProjectName}:build`,compodoc:useCompodoc,...useCompodoc&&{compodocArgs:["-e","json","-d",root||"."]}};architect.storybook||(architect.storybook={builder:"@storybook/angular:start-storybook",options:{...baseOptions,port:6006}}),architect["build-storybook"]||(architect["build-storybook"]={builder:"@storybook/angular:build-storybook",options:{...baseOptions,outputDir:Object.keys(this.projects).length===1?"storybook-static":`dist/storybook/${angularProjectName}`}})}write(){fs2.writeFileSync(ANGULAR_JSON_PATH,JSON.stringify(this.json,null,2))}};var generator=async(packageManager,npmOptions,options,commandOptions)=>{let packageJson=packageManager.retrievePackageJson(),angularVersionFromDependencies=semver2.coerce(packageManager.retrievePackageJson().dependencies["@angular/core"])?.version,angularVersionFromDevDependencies=semver2.coerce(packageManager.retrievePackageJson().devDependencies["@angular/core"])?.version,angularVersion=angularVersionFromDependencies||angularVersionFromDevDependencies,updatedOptions=semver2.gte(angularVersion,"12.0.0")?{...options,builder:"webpack5"}:options,angularJSON=new AngularJSON;if(angularJSON.projectsWithoutStorybook.length===0)return paddedLog("Every project in your workspace is already set up with Storybook. There is nothing to do!"),Promise.reject();let angularProjectName=await angularJSON.getProjectName();paddedLog(`Adding Storybook support to your "${angularProjectName}" project`);let{root}=angularJSON.getProjectSettingsByName(angularProjectName),{projects}=angularJSON,useCompodoc=commandOptions.yes?!0:await promptForCompoDocs(),storybookFolder=root?`${root}/.storybook`:".storybook";if(root!==""){fs3.mkdirSync(storybookFolder,{recursive:!0});let rootReferencePathFromStorybookFolder=root.split("/").map(()=>"../").join("");fs3.writeFileSync(`${storybookFolder}/main.ts`,dedent4(`
    ...
    `},async run({result:{main,framework},dryRun}){logger5.info(`\u2705 Setting 'framework' to '${framework}' in main.js`),dryRun||(main.setFieldValue(["framework"],framework),await writeConfig2(main))}};import chalk7 from"chalk";import{dedent as dedent10}from"ts-dedent";import{readConfig as readConfig3,writeConfig as writeConfig3}from"@storybook/csf-tools";import{getStorybookInfo as getStorybookInfo3}from"@storybook/core-common";import{readFile,readJson,writeJson}from"fs-extra";import detectIndent from"detect-indent";import fse3 from"fs-extra";var SUPPORTED_ESLINT_EXTENSIONS=["js","cjs","json"],UNSUPPORTED_ESLINT_EXTENSIONS=["yaml","yml"],findEslintFile=()=>{let filePrefix=".eslintrc",unsupportedExtension=UNSUPPORTED_ESLINT_EXTENSIONS.find(ext=>fse3.existsSync(`${filePrefix}.${ext}`));if(unsupportedExtension)throw new Error(unsupportedExtension);let extension=SUPPORTED_ESLINT_EXTENSIONS.find(ext=>fse3.existsSync(`${filePrefix}.${ext}`));return extension?`${filePrefix}.${extension}`:null};var logger6=console,eslintPlugin={id:"eslintPlugin",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{dependencies,devDependencies}=packageJson,eslintPluginStorybook=dependencies["eslint-plugin-storybook"]||devDependencies["eslint-plugin-storybook"],eslintDependency=dependencies.eslint||devDependencies.eslint;if(eslintPluginStorybook||!eslintDependency)return null;let{mainConfig}=getStorybookInfo3(packageJson);if(!mainConfig)return logger6.warn("Unable to find storybook main.js config, skipping"),null;let eslintFile,unsupportedExtension;try{eslintFile=findEslintFile()}catch(err){unsupportedExtension=err.message}if(!eslintFile&&!unsupportedExtension)return logger6.warn("Unable to find .eslintrc config file, skipping"),null;let main=await readConfig3(mainConfig);return{eslintFile,main,unsupportedExtension}},prompt(){return dedent10`
      ...
      More info: ${chalk7.yellow("https://github.com/storybookjs/eslint-plugin-storybook#readme")}
    `},async run({result:{eslintFile,unsupportedExtension},packageManager,dryRun}){let deps=["eslint-plugin-storybook"];if(logger6.info(`\u2705 Adding dependencies: ${deps}`),dryRun||packageManager.addDependencies({installAsDevDependencies:!0},deps),!dryRun&&unsupportedExtension){logger6.info(dedent10`
...
`:""}
      ...
      `),dedent14`
      We've detected you are using Storybook in a ${chalk11.bold("Next.js")} project.

      In Storybook 7, we introduced a new framework package for Next.js projects: ${chalk11.magenta("@storybook/nextjs")}.

      This package is a replacement for ${chalk11.magenta("@storybook/react-webpack5")} and provides a better experience for Next.js users.
      ${addonsMessage}
      To learn more about it, see: ${chalk11.yellow("https://github.com/storybookjs/storybook/blob/next/code/frameworks/nextjs/README.md")}
    `},async run({result:{addonsToRemove,main,frameworkOptions,packageJson},packageManager,dryRun}){let dependenciesToRemove=[...addonsToRemove,"@storybook/react-webpack5"];if(dependenciesToRemove.length>0&&(logger10.info(`\u2705 Removing redundant packages: ${dependenciesToRemove.join(", ")}`),!dryRun)){packageManager.removeDependencies({skipInstall:!0,packageJson},dependenciesToRemove);let updatedAddons=main.getFieldValue(["addons"]).filter(addon=>typeof addon=="string"?!addonsToRemove.includes(addon):addon.name?!addonsToRemove.includes(addon.name):!1);main.setFieldValue(["addons"],updatedAddons)}if(logger10.info("\u2705 Installing new dependencies: @storybook/nextjs"),!dryRun){let versionToInstall=getStorybookVersionSpecifier(packageJson);packageManager.addDependencies({installAsDevDependencies:!0,packageJson},[`@storybook/nextjs@${versionToInstall}`])}logger10.info("\u2705 Updating framework field in main.js"),dryRun||(main.setFieldValue(["framework","options"],frameworkOptions),main.setFieldValue(["framework","name"],"@storybook/nextjs"),await writeConfig5(main))}};import chalk12 from"chalk";import dedent15 from"ts-dedent";import semver12 from"semver";import{readConfig as readConfig6,writeConfig as writeConfig6}from"@storybook/csf-tools";import{getStorybookInfo as getStorybookInfo8}from"@storybook/core-common";var logger11=console,packagesMap={"@storybook/react":{webpack5:"@storybook/react-webpack5",vite:"@storybook/react-vite"},"@storybook/preact":{webpack5:"@storybook/preact-webpack5",vite:"@storybook/preact-vite"},"@storybook/server":{webpack5:"@storybook/server-webpack5"},"@storybook/angular":{webpack5:"@storybook/angular"},"@storybook/vue":{webpack5:"@storybook/vue-webpack5",vite:"@storybook/vue-vite"},"@storybook/vue3":{webpack5:"@storybook/vue3-webpack5",vite:"@storybook/vue3-vite"},"@storybook/svelte":{webpack5:"@storybook/svelte-webpack5",vite:"@storybook/svelte-vite"},"@storybook/web-components":{webpack5:"@storybook/web-components-webpack5",vite:"@storybook/web-components-vite"},"@storybook/html":{webpack5:"@storybook/html-webpack5",vite:"@storybook/html-vite"}},getBuilder=builder=>typeof builder=="string"?builder.includes("vite")?"vite":"webpack5":builder?.name.includes("vite")?"vite":"webpack5",getFrameworkOptions=(framework,main)=>{let frameworkOptions={};try{frameworkOptions=main.getFieldValue([`${framework}Options`])}catch(e){logger11.warn(dedent15`
    ...
    `),console.log(e)}return frameworkOptions||{}},newFrameworks={id:"newFrameworks",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),allDeps={...packageJson.dependencies,...packageJson.devDependencies},{mainConfig,version:storybookVersion,framework}=getStorybookInfo8(packageJson);if(!mainConfig)return logger11.warn("Unable to find storybook main.js config, skipping"),null;let storybookCoerced=storybookVersion&&semver12.coerce(storybookVersion)?.version;if(!storybookCoerced)throw new Error(dedent15`
      ...
      `);if(!semver12.gte(storybookCoerced,"7.0.0"))return null;let main=await readConfig6(mainConfig),frameworkPackage=main.getNameFromPath(["framework"]),builder=main.getFieldValue(["core","builder"]);if(!frameworkPackage||!Object.keys(packagesMap).includes(frameworkPackage))return null;let builderInfo={name:getBuilder(builder),options:main.getFieldValue(["core","builder","options"])||{}},newFrameworkPackage=packagesMap[frameworkPackage][builderInfo.name];if(!newFrameworkPackage)return null;let frameworkOptions=newFrameworkPackage==="@storybook/svelte-vite"?{}:getFrameworkOptions(framework,main),dependenciesToRemove=["@storybook/builder-webpack5","@storybook/manager-webpack5","@storybook/builder-webpack4","@storybook/manager-webpack4","@storybook/builder-vite","storybook-builder-vite"].filter(dep=>allDeps[dep]),dependenciesToAdd=[];if(newFrameworkPackage!==frameworkPackage&&dependenciesToAdd.push(newFrameworkPackage),allDeps.vite&&semver12.lt(semver12.coerce(allDeps.vite).version,"3.0.0"))throw new Error(dedent15`
        ❌ Your project should be upgraded to use the framework package ${chalk12.bold(newFrameworkPackage)}, but we detected that you are using Vite ${chalk12.bold(allDeps.vite)}, which is unsupported in ${chalk12.bold("Storybook 7.0")}. Please upgrade Vite to ${chalk12.bold("3.0.0 or higher")} and rerun this migration.
      ...
      We can remove the dependencies that are no longer needed: ${chalk12.yellow(dependenciesToRemove.join(", "))}
      
      And set up the ${chalk12.magenta(frameworkPackage)} framework that already includes the builder.

      To learn more about the framework field, see: ${chalk12.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#framework-field-mandatory")}

      ${chalk12.underline(chalk12.bold(chalk12.cyan("Webpack4 users")))}
...
`)}
    ...
    `}};import chalk14 from"chalk";import{dedent as dedent17}from"ts-dedent";import{basename}from"path";import fse4 from"fs-extra";import globby from"globby";var MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger12=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await globby("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return dedent17`
    ...
    `},async run({result:{storiesMdxFiles},dryRun}){await Promise.all([...storiesMdxFiles.map(async fname=>{let contents=await fse4.readFile(fname,"utf-8"),updated=fixMdxComments(fixMdxStyleTags(contents));updated===contents?logger12.info(`\u{1F197} Unmodified ${basename(fname)}`):(logger12.info(`\u2705 Modified ${basename(fname)}`),dryRun||await fse4.writeFile(fname,updated))})])}};import chalk15 from"chalk";import{dedent as dedent18}from"ts-dedent";import{readConfig as readConfig7,writeConfig as writeConfig7}from"@storybook/csf-tools";import{getStorybookInfo as getStorybookInfo10}from"@storybook/core-common";var logger13=console,autodocsTrue={id:"autodocsTrue",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig}=getStorybookInfo10(packageJson);if(!mainConfig)return logger13.warn("Unable to find storybook main.js config, skipping"),null;let main=await readConfig7(mainConfig),docs=main.getFieldValue(["docs"]),docsPageToAutodocsMapping={true:"tag",automatic:!0,false:!1};if(docs?.docsPage){let oldValue=docs?.docsPage.toString();if(!(oldValue in docsPageToAutodocsMapping))throw new Error(`Unexpected value for docs.docsPage: ${oldValue}`);return{main,value:docsPageToAutodocsMapping[oldValue]}}return docs?.autodocs===void 0?{main}:null},prompt({value}){let autodocsFormatted=chalk15.cyan(`docs: { autodocs: ${JSON.stringify(value??!0)} }`);return value?dedent18`
    ...
    `:dedent18`
      ...
      More info: ${chalk15.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#autodocs")}
    `},async run({result:{main,value},dryRun}){logger13.info("\u2705 Setting 'docs.autodocs' to true in main.js"),dryRun||(main.removeField(["docs","docsPage"]),main.setFieldValue(["docs","autodocs"],value??!0),await writeConfig7(main))}};import chalk16 from"chalk";import dedent19 from"ts-dedent";import semver13 from"semver";import{readConfig as readConfig8,writeConfig as writeConfig8}from"@storybook/csf-tools";import{getStorybookInfo as getStorybookInfo11}from"@storybook/core-common";var logger14=console,fixId="sveltekitFramework",sveltekitFramework={id:fixId,async check({packageManager}){let packageJson=packageManager.retrievePackageJson();if(!{...packageJson.dependencies,...packageJson.devDependencies}["@sveltejs/kit"])return null;let{mainConfig,version:storybookVersion}=getStorybookInfo11(packageJson);if(!mainConfig)return logger14.warn("Unable to find storybook main.js config, skipping"),null;let sbVersionCoerced=storybookVersion&&semver13.coerce(storybookVersion)?.version;if(!sbVersionCoerced)throw new Error(dedent19`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory?
      `);if(!semver13.gte(sbVersionCoerced,"7.0.0"))return null;let main=await readConfig8(mainConfig),framework=main.getNameFromPath(["framework"]);if(!framework)return logger14.warn(dedent19`
      ❌ Unable to determine Storybook framework, skipping ${chalk16.cyan(fixId)} fix.
      🤔 Are you running automigrate from your project directory?
    `),null;if(framework==="@storybook/sveltekit")return null;if(framework==="@storybook/svelte-vite")return{main,packageJson,dependenciesToRemove:["@storybook/svelte-vite"]};if(framework!=="@storybook/svelte")return logger14.warn(dedent19`
            ...
            If you are interested in using this package, see: ${chalk16.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#sveltekit-needs-the-storybooksveltekit-framework")}
          `),null;let builder=main.getFieldValue(["core","builder"]);return["@storybook/builder-vite","storybook-builder-vite"].includes(builder)?{main,packageJson,dependenciesToRemove:[builder]}:(logger14.warn(dedent19`
          We've detected you are using Storybook in a SvelteKit project.
...
`);return dedent20`
    ...
    `}};import chalk19 from"chalk";import dedent22 from"ts-dedent";import semver15 from"semver";import{getStorybookInfo as getStorybookInfo13}from"@storybook/core-common";import{loadPartialConfigAsync}from"@babel/core";import{readConfig as readConfig9}from"@storybook/csf-tools";var logger15=console,frameworksThatNeedBabelConfig=["@storybook/react-webpack5","@storybook/vue-webpack5","@storybook/vue3-webpack5","@storybook/html-webpack5"],missingBabelRc={id:"missing-babelrc",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig,version:storybookVersion}=getStorybookInfo13(packageJson),storybookCoerced=storybookVersion&&semver15.coerce(storybookVersion)?.version;if(!storybookCoerced)throw new Error(dedent22`
      ...
      We can create a ${chalk19.blue(".babelrc.json")} file with some basic configuration and add any necessary package devDependencies.

      Please see the migration guide for more information:
      ${chalk19.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#babel-mode-v7-exclusively")}
    `},async run(){logger15.info(),await generateStorybookBabelConfigInCWD()}};import chalk20 from"chalk";import dedent23 from"ts-dedent";import semver16 from"semver";import{readConfig as readConfig10,writeConfig as writeConfig9}from"@storybook/csf-tools";import{getStorybookInfo as getStorybookInfo14}from"@storybook/core-common";var logger16=console,fixId2="bare-mdx-stories-glob",getNextGlob=glob=>{let extGlobsRegex=new RegExp(/(.*\.)(stories\.@.*)(\|mdx|mdx\|)(.*)$/i);if(glob.match(extGlobsRegex))return glob.replace(extGlobsRegex,"$1@(mdx|$2$4)");let allStoriesExtensionsRegex=new RegExp(/(.*\.)(stories\.\*)$/i);return glob.match(allStoriesExtensionsRegex)?glob.replace(allStoriesExtensionsRegex,"$1@(mdx|$2)"):glob.replaceAll(".stories.mdx",".mdx")},bareMdxStoriesGlob={id:fixId2,async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig,version:storybookVersion}=getStorybookInfo14(packageJson);if(!mainConfig)return logger16.warn("Unable to find storybook main.js config, skipping"),null;let sbVersionCoerced=storybookVersion&&semver16.coerce(storybookVersion)?.version;if(!sbVersionCoerced)throw new Error(dedent23`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory?
      `);if(!semver16.gte(sbVersionCoerced,"7.0.0"))return null;let main=await readConfig10(mainConfig),existingStoriesEntries;try{existingStoriesEntries=main.getFieldValue(["stories"])}catch{}if(!existingStoriesEntries)throw new Error(dedent23`
      ❌ Unable to determine Storybook stories globs, skipping ${chalk20.cyan(fixId2)} fix.
      ...
      To learn more about this change, see: ${chalk20.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
...
`);return dedent23`
    We've detected your project has one or more globs in your 'stories' config that matches .stories.mdx files:
      ${chalk20.cyan(prettyExistingStoriesEntries)}
    ...
    That would result in the following 'stories' config:
      ${chalk20.cyan(prettyNextStoriesEntries)}

    To learn more about this change, see: ${chalk20.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,result:{nextStoriesEntries,main}}){logger16.info(dedent23`✅ Setting 'stories' config:
...
`),codeLog(["sb init [options] -f"]),logger18.log();return}let installResult=await installStorybook(projectType,packageManager,options).catch(e=>{process.exit()});!options.skipInstall&&!storybookInstalled&&packageManager.installDependencies(),options.disableTelemetry||telemetry("init",{projectType}),projectType!=="REACT_NATIVE"&&await automigrate({yes:options.yes||process.env.CI==="true",packageManager:pkgMgr}),logger18.log(`
For more information visit:`,chalk22.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger18.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger18.log(),logger18.log(chalk22.yellow(`NOTE: installation is not 100% automated.
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger22.info(boxen2(dedent25`
            ...
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),selectedTemplate=await promptSelectedTemplate(choices)),!!!(selectedTemplate??null)){logger22.error("Somehow we got no templates. Please rerun this command!");return}if(selectedConfig=allTemplates[selectedTemplate],!selectedConfig)throw new Error("\u{1F6A8} Sandbox: please specify a valid template type")}let selectedDirectory=outputDirectory,outputDirectoryName=outputDirectory||selectedTemplate;if(selectedDirectory&&existsSync(`${selectedDirectory}`)&&logger22.info(`\u26A0\uFE0F  ${selectedDirectory} already exists! Overwriting...`),!selectedDirectory){let{directory}=await prompts5({type:"text",message:"Enter the output directory",name:"directory",initial:outputDirectoryName,validate:async directoryName=>existsSync(directoryName)?`${directoryName} already exists. Please choose another name.`:!0},{onCancel:()=>{logger22.log("Command cancelled by the user. Exiting..."),process.exit(1)}});selectedDirectory=directory}try{let templateDestination=path6.isAbsolute(selectedDirectory)?selectedDirectory:path6.join(process.cwd(),selectedDirectory);logger22.info(`\u{1F3C3} Adding ${selectedConfig.name} into ${templateDestination}`),logger22.log("\u{1F4E6} Downloading sandbox template...");try{let gitPath=`github:storybookjs/sandboxes/${selectedTemplate}/${init?"after-storybook":"before-storybook"}#${branch}`;if(await downloadTemplate(gitPath,{force:!0,dir:templateDestination}),(await readdir(templateDestination)).length===0)throw new Error(dedent25`Template downloaded from ${chalk23.blue(gitPath)} is empty.
          Are you use it exists? Or did you want to set ${chalk23.yellow(selectedTemplate)} to inDevelopment first?`)}catch(err){throw logger22.error(`\u{1F6A8} Failed to download sandbox template: ${err.message}`),err}let initMessage=init?chalk23.yellow(`yarn install
yarn storybook`):`Recreate your setup, then ${chalk23.yellow("run npx storybook init")}`;logger22.info(boxen2(dedent25`
